<template>
  <div class="basket-tov-block">
    <div class="basket-tov" v-for="(item, idx) in cart_data_tov" :key="idx">
      ...
      <div class="basket-tov-desk">
        <div class="basket-tov-desk-left">
          ...
          <div class="basket-tov-desk-type">
            <strong>{{
              item.product[0].filter_show_name
                ? item.product[0].filter_show_name
                : ""
            }}</strong>
            <span
              >{{
                item.product[0].filter_show_value
                  ? item.product[0].filter_show_value
                  : ""
              }}<span></span
            ></span>
          ...
          <el-input-number
            ...
            @change="
              changeQuantyty(
                item.id,
                item.product[0].id,
                item.product[0].count_el,
                item.product[0].discont != null
        ...
        <div class="basket-tov-dell">
          <i @click="cartDell(idx)" class="el-icon-delete"></i>
        </div>
      </div>
    </div>
    <div class="basket-itog">
        ...
        <div class="sale-icon-div">
          ...
          >
            <el-button
              v-show="
                user_discount > 0 &&
                user_discount >= for_amount_discount_persent
              "
              slot="reference"
              type="danger"
              circle
        ...
        <el-collapse accordion>
          <el-collapse-item name="1">
            ...
            <div class="basket-diskont-inf-block dis-block">
              <h5>
                В Архитерм действует система скидок за объем от суммы заказа.
              </h5>
              <span
                >Чем на большую сумму вы совершаете заказ – тем большую скидку
                на него вы получаете!
              </span>
              <p>
      ...
      <div class="basket-itog-summ">
        ...
        <span v-if="for_amount_none === 0">Итого:</span>
...
export default {
  ...
  methods: {
    cartDell(idx) {
      ...
      this.amount();
      this.$emit("basketCostUpdate", {
        for_amount: this.for_amount,
        for_amount_discount: this.for_amount_discount,
        for_amount_discount_persent: this.for_amount_discount_persent,
        for_amount_none: this.for_amount_none,
        for_amount_itog: this.for_amount_itog,
      });
    },
      ...
      for (let i in this.discount_arr) {
        if (this.for_amount >= Number([i])) {
          if (
            ...
            this.user_discount > this.discount_arr[i]
          ) {
          ...
          this.for_amount_discount =
            (this.for_amount * this.for_amount_discount_persent) / 100;
          this.for_amount_discount =
            Math.ceil(this.for_amount_discount * 100) / 100;
