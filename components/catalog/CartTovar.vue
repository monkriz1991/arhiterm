        ...
        <div class="catalog-list-block">
          <nuxt-link
            ...
            :to="{
              ...
              params: {
                product: `${product.kirilica_name}`,
                id: `${product.kirilica_name}`,
              },
            }"
          >
            ...
            <div class="catalog-list-block-desc">
              ...
              <div class="catalog-list-block-price">
                <b
                  v-if="
                    product.product.length > 1 && product.product.length != 0
                  "
                  >от</b
                >
                <strong :class="{ butDiscountNone: discont[idx] }">
                  {{ price[idx] }}
                </strong>
                <span v-if="product.product.length != 0"
                  >руб/{{ product.units }}</span
                >
              </div>
            </div>
          </nuxt-link>
...
export default {
  ...
  methods: {
        ...
        for (let item in this.categoriesNested.list_filter) {
          for (let itemin in this.categoriesNested.list_filter[item].chice) {
            for (let fill in this.tabsArr.card_filter_rus) {
              if (
                this.tabsArr.card_filter_rus[fill] ==
                this.categoriesNested.list_filter[item].chice[itemin].value
              ) {
                filter +=
                  this.categoriesNested.list_filter[item].chice[itemin]
                    .kirilica + ",";
              }
            }
          }
      ...
      if (this.tabsArr.manuf_filter_rus != "") {
        for (let item in this.categoryManuf.results) {
          for (let items in this.tabsArr.manuf_filter_rus) {
            if (
              this.tabsArr.manuf_filter_rus[items] ==
              this.categoryManuf.results[item].name
            ) {
    ...
    checkFilRoute(FilterItem, ManufItem) {
      if (this.$route.name == "catalog-catalog") {
        this.$router.replace({
          ...
          params: { catalog: this.$route.params.catalog },
          query: { card_filter: FilterItem, manuf: ManufItem },
        });
      } else if (this.$route.name == "factory-factory") {
        this.$router.replace({
          name: "factory-factory",
          params: { factory: this.$route.params.factory },
          query: { card_filter: FilterItem },
        });
      }
    },
    hidePreload(item) {
    ...
    addToCart(data) {
      let cart = JSON.parse(
        JSON.stringify(this.productsList.find((i) => i.id == data.data.parent))
      );
