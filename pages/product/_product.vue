<template>
  <div class="container">
    ...
    <div itemscope itemtype="https://schema.org/Product">
      ...
      <div class="gal-parent">
        <Galery :productItems="productItem" />
        <div class="prod-doc" v-if="!document.length == 0">
          <a :href="`https://new.arhiterm.by` + document[0].doc" target="_blank"
            ><i class="el-icon-document"></i> PDF</a
          >
        </div>
      </div>
        ...
        <div class="dilivery-cart-block">
          ...
          <div class="dilivery-cart">
            <el-drawer
              ...
              :with-header="false"
            >
              <Drawerkont :visible.sync="drawer" />
            </el-drawer>
            <el-button
      ...
      <div v-show="stufProduct.length" class="stus-product">
        <StufProduct :stufProduct="stufProduct" />
      </div>
    </div>
    <client-only>
      <BasketModal
...
export default {
  async asyncData({ app, params, route, error }) {
    try {
      let result = await app.store.dispatch(
        "tovar/getProductItem",
        params.product
      );
      let price = result.product[0].price;
      let discont = result.product[0].discont;
      let kodProduct = result.product[0].name;
      let document = result.docs;

      return { price, discont, result, kodProduct, document };
    } catch (err) {
      return error({
        statusCode: 404,
